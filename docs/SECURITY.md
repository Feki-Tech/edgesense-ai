...
| S6 | Model artifact | `joblib.load` of a pickle; artifacts now also arrive from the registry at runtime (`inference/registry.py`) | Artifact substitution = arbitrary code execution in the inference container | **Partly mitigated:** Ed25519 signature verified before load (`ml/signing.py`, MLOPS §2.7). Remaining: ONNX-only load path (P4) |
...
| | Swap the model artifact (F3) | Signature verified before load when a trust store is configured; `EDGESENSE_REQUIRE_SIGNATURE` makes it mandatory | Enforce it everywhere + ONNX-only loading (P4) |
...
| **P4 — Signed models, pickle-free inference** | ~~Sign model artifacts at training; verify at load~~ **(shipped — `ml/signing.py`, MLOPS §2.7)**; still to do: serve from ONNX only (no `joblib.load` in the serving path), and give CI/the AML retrain job a signing key so automated promotions are signed | `ml/train.py`, `ml/export_onnx.py`; `inference/server.py`, `inference/Dockerfile`; `.github/workflows/ci.yml` | P1 | Inference refuses an unsigned/modified artifact; no pickle deserialization at serve time |
